...
## 电脑↔手机数据同步（可选）
本静态站默认数据存于各自浏览器，**不跨设备**。如需两端自动同步，按
[SUPABASE_SETUP.md](./SUPABASE_SETUP.md) 建一个免费 Supabase 项目（2 分钟），
然后在 App 右上角「☁ 同步」里填 URL + Anon Key、登录同一账号即可。
同步仅含「生活记录」与「FIRE 规划」；行情各自实时拉取。
